...
public class StoneGen {
    ...
    private static void doStoneReplacements(BlockEvent.FluidPlaceBlockEvent event, BlockState catalyst) {
        if (Configuration.VANILLA_VARIANTS_AS_STONE.get()) {
    ...
    private static void doCobblestoneReplacements(BlockEvent.FluidPlaceBlockEvent event, BlockState catalyst) {
        if (Configuration.VANILLA_VARIANTS_AS_COBBLE.get()) {
            replaceBlock(event, catalyst, Blocks.GRANITE.getDefaultState());
            replaceBlock(event, catalyst, Blocks.DIORITE.getDefaultState());
            replaceBlock(event, catalyst, Blocks.ANDESITE.getDefaultState());
        }
    ...
    public static void getCustomCatalysts() {
        ...
        if(ModList.get().isLoaded("quark") && Configuration.ENABLE_QUARK_COMPAT.get() && Configuration.QUARK_VARIANTS_AS_STONE.get()) {
            customStoneCatalysts.add(NewStoneTypesModule.marbleBlock);
            customStoneCatalysts.add(NewStoneTypesModule.limestoneBlock);
            customStoneCatalysts.add(NewStoneTypesModule.jasperBlock);
            customStoneCatalysts.add(NewStoneTypesModule.slateBlock);
            customStoneCatalysts.add(NewStoneTypesModule.basaltBlock);
        }

        if(ModList.get().isLoaded("quark") && Configuration.ENABLE_QUARK_COMPAT.get() && Configuration.QUARK_VARIANTS_AS_COBBLE.get()) {
            customCobbleCatalysts.add(NewStoneTypesModule.marbleBlock);
            customCobbleCatalysts.add(NewStoneTypesModule.limestoneBlock);
            customCobbleCatalysts.add(NewStoneTypesModule.jasperBlock);
            customCobbleCatalysts.add(NewStoneTypesModule.slateBlock);
            customCobbleCatalysts.add(NewStoneTypesModule.basaltBlock);
        }
    }
